...
export async function generateMeetingDetailsPDF(meeting: any): Promise<Buffer> {
  ...
  if (meeting.meetingLink && (meeting.meetingMode === 'online' || meeting.meetingMode === 'hybrid')) {
    ...
    for (const line of linkLines) { page.drawText(line, { x: leftMargin + 100, y: yPosition, size: 12, font: font, color: rgb(0, 0.4, 0.8) }); yPosition -= 15; }
  }
...
export async function generateAgendaPDF(meeting: any, agendaData: any): Promise<Buffer> {
  ...
  const pageWidth = 595;
  const lineHeight = 20;
  const checkAndAddPage = (requiredSpace: number) => { if (yPosition - requiredSpace < 50) { page = pdfDoc.addPage([595, 842]); yPosition = 750; return true; } return false; };
  ...
  if (agendaData.objectives) {
    ...
    for (const line of objectiveLines) { checkAndAddPage(lineHeight); page.drawText(line, { x: leftMargin + 10, y: yPosition, size: 12, font: font, color: rgb(0, 0, 0) }); yPosition -= lineHeight; }
    yPosition -= 10;
  }
  if (agendaData.preparationRequired && agendaData.preparationRequired.length > 0) {
    ...
    agendaData.preparationRequired.forEach((item: string, index: number) => {
      ...
      const itemLines = wrapText(${index + 1}. , pageWidth - leftMargin - 60, 12, font);
      for (const line of itemLines) { checkAndAddPage(lineHeight); page.drawText(line, { x: leftMargin + 10, y: yPosition, size: 12, font: font, color: rgb(0, 0, 0) }); yPosition -= lineHeight; }
    });
    yPosition -= 10;
  }
  if (agendaData.agendaItems && agendaData.agendaItems.length > 0) {
    ...
    agendaData.agendaItems.forEach((item: any, index: number) => {
      checkAndAddPage(80);
      page.drawText(${index + 1}. , { x: leftMargin + 10, y: yPosition, size: 12, font: boldFont, color: rgb(0, 0, 0) });
      yPosition -= lineHeight;
      if (item.description) {
        const descLines = wrapText(item.description, pageWidth - leftMargin - 70, 11, font);
        for (const line of descLines) { checkAndAddPage(lineHeight); page.drawText(line, { x: leftMargin + 20, y: yPosition, size: 11, font: font, color: rgb(0.2, 0.2, 0.2) }); yPosition -= lineHeight; }
      }
      if (item.presenter || item.duration) {
        checkAndAddPage(lineHeight);
        const details = [];
        if (item.presenter) details.push(Presenter: );
        if (item.duration) details.push(Duration:  min);
...
export async function generateMinutesPDF(meeting: any, minutesData: any): Promise<Buffer> {
  ...
  if (typeof minutesData === 'string') { try { const parsed = JSON.parse(minutesData); if (parsed.savedMinutes && parsed.savedMinutes.length > 0) { parsedMinutes = parsed.savedMinutes[parsed.savedMinutes.length - 1]; } } catch (e) { parsedMinutes = {}; } }
  if (parsedMinutes?.attendees) {
    ...
    for (const line of attendeeLines) { checkAndAddPage(lineHeight); page.drawText(line, { x: leftMargin + 10, y: yPosition, size: 12, font: font, color: rgb(0, 0, 0) }); yPosition -= lineHeight; }
    yPosition -= 10;
  }
  if (parsedMinutes?.discussion) {
    ...
    for (const line of discussionLines) { checkAndAddPage(lineHeight); page.drawText(line, { x: leftMargin + 10, y: yPosition, size: 12, font: font, color: rgb(0, 0, 0) }); yPosition -= lineHeight; }
    yPosition -= 10;
  }
  if (parsedMinutes?.decisions) {
    ...
    for (const line of decisionLines) { checkAndAddPage(lineHeight); page.drawText(line, { x: leftMargin + 10, y: yPosition, size: 12, font: font, color: rgb(0, 0, 0) }); yPosition -= lineHeight; }
    yPosition -= 10;
  }
  ...
  if (parsedMinutes?.notes) {
    ...
    for (const line of notesLines) { checkAndAddPage(lineHeight); page.drawText(line, { x: leftMargin + 10, y: yPosition, size: 12, font: font, color: rgb(0, 0, 0) }); yPosition -= lineHeight; }
  }
  const pdfBytes = await pdfDoc.save();
  return Buffer.from(pdfBytes);
}
export async function generatePDFFromHTML(htmlContent: string, title: string): Promise<Buffer> {
  ...
  for (const paragraph of paragraphs) {
    ...
    for (const line of lines) { checkAndAddPage(lineHeight); page.drawText(line, { x: leftMargin, y: yPosition, size: 12, font: font, color: rgb(0, 0, 0) }); yPosition -= lineHeight; }
    yPosition -= 5;
  }
  const pdfBytes = await pdfDoc.save();
  return Buffer.from(pdfBytes);
