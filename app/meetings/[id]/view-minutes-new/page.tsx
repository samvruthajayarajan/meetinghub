        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
              <button onClick={() => router.push('/user')} className="text-gray-600 hover:text-black transition-colors flex-shrink-0">
            ...
            <button onClick={() => router.push(`/meetings/${resolvedParams.id}/minutes`)} className="px-3 py-2 sm:px-4 sm:py-2 bg-green-600 hover:bg-green-700 text-white text-xs sm:text-sm font-medium rounded-lg transition-colors flex items-center gap-1 sm:gap-2 shadow-sm">
      ...
      <main className="max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 space-y-4 sm:space-y-6">
        ...
        {savedMinutes.length > 0 ? (
                ...
                <div key={minutes.id} className="bg-white rounded-lg border border-gray-200 hover:border-blue-300 transition-all shadow-sm">
                  <div className="p-4">
                    <div className="flex flex-col sm:flex-row items-start gap-3">
                            ...
                            {minutes.decisions && minutes.decisions.length > 0 && (
                              <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                                ...
                                <ul className="list-disc list-inside text-gray-700 text-xs sm:text-sm space-y-1">
                                  {minutes.decisions.map((decision, idx) => <li key={idx}>{decision}</li>)}
                                </ul>
                              </div>
                            )}
                      ...
                      <button onClick={() => setViewingMinutesId(viewingMinutesId === minutes.id ? null : minutes.id)} className="w-full sm:w-auto px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-12 text-center shadow-sm">
            ...
            <button onClick={() => router.push(`/meetings/${resolvedParams.id}/minutes`)} className="px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-medium rounded-xl transition-colors">
              Create Minutes
            </button>
          </div>
        )}
        ...
        {savedMinutes.length > 0 && (
          <div className="bg-gradient-to-br from-blue-50 via-white to-gray-50 border border-gray-200 rounded-2xl p-4 sm:p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 max-w-2xl mx-auto">
              <button onClick={() => setShowEmailModal(true)} disabled={sending} className="group relative p-4 sm:p-6 bg-white hover:bg-blue-50 border-2 border-gray-200 hover:border-blue-300 rounded-xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
              ...
              <button onClick={() => setShowWhatsAppModal(true)} disabled={sending} className="group relative p-4 sm:p-6 bg-white hover:bg-green-50 border-2 border-gray-200 hover:border-green-300 rounded-xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                <div className="flex flex-col items-center gap-2 sm:gap-3">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
            ...
            <div className="p-8">
              ...
              <textarea value={emailRecipients} onChange={(e) => setEmailRecipients(e.target.value)} rows={4} className="w-full px-4 py-3.5 bg-gray-50 border-2 border-gray-300 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none mb-6" placeholder="Enter email addresses (comma separated)&#10;example@email.com, another@email.com" />
              <div className="flex gap-3">
                <button onClick={handleSendEmail} disabled={sending} className="flex-1 px-6 py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl transition-all duration-300 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                  {sending ? 'Sending...' : 'Send Email'}
                </button>
                <button onClick={() => { setShowEmailModal(false); setEmailRecipients(''); }} disabled={sending} className="px-6 py-3.5 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold rounded-xl transition-all duration-300 disabled:opacity-50">
                  Cancel
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
            ...
            <div className="p-8">
              ...
              <textarea value={whatsappNumber} onChange={(e) => setWhatsappNumber(e.target.value)} rows={4} className="w-full px-4 py-3.5 bg-gray-50 border-2 border-gray-300 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all resize-none font-mono text-sm mb-6" placeholder="+1234567890&#10;+919876543210&#10;+447123456789" />
              <div className="flex gap-3">
                <button onClick={handleSendWhatsApp} disabled={sending} className="flex-1 px-6 py-3.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold rounded-xl transition-all duration-300 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                  {sending ? 'Processing...' : 'Send WhatsApp'}
                </button>
                <button onClick={() => { setShowWhatsAppModal(false); setWhatsappNumber(''); }} disabled={sending} className="px-6 py-3.5 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold rounded-xl transition-all duration-300 disabled:opacity-50">
                  Cancel
                </button>
              </div>
            </div>
